...
struct MarblesWidget : ModuleWidget {
	...
	void appendContextMenu(Menu* menu) override {
		...
		menu->addChild(createIndexSubmenuItem("Y divider ratio", {
			"1/64",
			"1/48",
			"1/32",
			"1/24",
		...
		[=]() {
			return module->y_divider_index;
		},
		[=](int mode) {
			module->y_divider_index = mode;
			module->params[module->Y_RATE_PARAM].setValue(mode / LENGTHOF(y_divider_ratios));
		}));


		menu->addChild(createIndexPtrSubmenuItem("Edit Mode",
		{"Default", "Y generator", "Gate"},
		&module->edit_mode
		));
	}
};
